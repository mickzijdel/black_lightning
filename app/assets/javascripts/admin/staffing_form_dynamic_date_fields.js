...
function initStaffingDateFields() {
  var addDateButton = document.getElementById('add_date');
  var dateBlueprint = document.getElementById('date_blueprint');

  // Only run on pages with the staffing form
  if (!addDateButton || !dateBlueprint) return;

  // Avoid duplicate listeners by checking if already initialized
  if (addDateButton.dataset.initialized) return;
  addDateButton.dataset.initialized = 'true';
...
document.addEventListener('DOMContentLoaded', initStaffingDateFields);
document.addEventListener('turbo:load', initStaffingDateFields);